...
class ModelCatalogCategory extends Model {
    ...
    public function changeStatus($category_id, $value) {
        $query = $this->db->query("UPDATE " . DB_PREFIX ."category SET status = " . (int)$value . " WHERE category_id = '" . (int)$category_id ."'");
        $queryProducts = $this->db->query("SELECT * FROM " . DB_PREFIX ."product_to_category WHERE category_id = '" . (int)$category_id ."'");
        foreach ($queryProducts->rows as $product) {
            $queryProduct = $this->db->query("UPDATE " . DB_PREFIX ."product SET status = " . (int)$value . " WHERE product_id = '" . (int)$product['product_id'] ."'");
        }
    }
}