...
class ControllerSettingSetting extends Controller
{
    ...
    public function index()
    {
        ...
        foreach($languages as $language){
            ...
            if (isset($this->request->post['config_name'.$id])) {
                $data['config_name'.$id] = $this->request->post['config_name'.$id];
            } else {
                $data['config_name'][$id] = $this->config->get('config_name'.$id);
            }

            if (isset($this->request->post['config_owner'.$id])) {
                $data['config_owner'.$id] = $this->request->post['config_owner'.$id];
            } else {
                $data['config_owner'][$id] = $this->config->get('config_owner'.$id);
            }

            if (isset($this->request->post['config_address'.$id])) {
                $data['config_address'.$id] = $this->request->post['config_address'.$id];
            } else {
                $data['config_address'][$id] = $this->config->get('config_address'.$id);
            }

            if (isset($this->request->post['config_open'.$id])) {
                $data['config_open'] = $this->request->post['config_open'.$id];
            } else {
                $data['config_open'][$id] = $this->config->get('config_open'.$id);
            }

            if (isset($this->request->post['config_comment'.$id])) {
                $data['config_comment'.$id] = $this->request->post['config_comment'.$id];
            } else {
                $data['config_comment'][$id] = $this->config->get('config_comment'.$id);
            }
        }
    ...
    protected function validate()
    {
        ...
        foreach ($languages as $language) {
            $id = $language['language_id'];
            if (!$this->request->post['config_name' . $id]) {
                $this->error['name' . $id] = $this->language->get('error_name');
            }

            if ((utf8_strlen($this->request->post['config_owner' . $id]) < 3) || (utf8_strlen($this->request->post['config_owner' . $id]) > 64)) {
                $this->error['owner' . $id] = $this->language->get('error_owner');
            }

            if ((utf8_strlen($this->request->post['config_address' . $id]) < 3) || (utf8_strlen($this->request->post['config_address' . $id]) > 256)) {
                $this->error['address' . $id] = $this->language->get('error_address');
            }
        }
...
//        if ((utf8_strlen($this->request->post['config_viber']) < 3) || (utf8_strlen($this->request->post['config_viber']) > 32)) {
//            $this->error['viber'] = $this->language->get('error_viber');
//        }
