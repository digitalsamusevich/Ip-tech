...
class ControllerCatalogCategory extends Controller {
	...
	protected function getList() {
		...
		$data['change'] = $this->url->link('catalog/category/change&user_token=' . $this->session->data['user_token'] . $url);
			...
			foreach ($this->request->post['category_seo_url'] as $store_id => $language) {
				foreach ($language as $language_id => $keyword) {
					if (!empty($keyword)) {
						...
						if (count(array_keys($language, $keyword)) > 2) {
    ...
    public function change() {
        ...
        foreach ($this->request->post['category_id'] as $category) {

            if (isset($this->request->post['status_' . $category])){
                $this->model_catalog_category->changeStatus($category, 1);
            }
            else {
                $this->model_catalog_category->changeStatus($category, 0);
            }
        }
